...
public final class GLVectorProgram extends GLProgram implements IGLUniformProvider.WorldMatrix , IGLUniformProvider.Scale, IGLUniformProvider.Color, GLBuffer.IXY_UV {
	...
	private static final String SOURCE_VERTEX_SHADER   = 
		...
		"\n"+"void main(void) { " +
		"\n"+"\t"+"vBezier     = aBezier;" +
		"\n"+"\t"+"gl_Position = (uProjectionMatrix * uViewMatrix * uModelMatrix) * vec4((uXScale * aPosition.x), (uYScale * aPosition.y), 0.0f, 1.0f);" +
		"\n"+"}";
	...
	private static final String SOURCE_FRAGMENT_SHADER = /** TODO: Fragment Precision. (Important for mobile devices, crashes @desktop etc). **/
		...
		"\n"+"\t"+"lReturnColor.a    *= (1.0f - lIsCurve) + (clamp((((float((vBezier.x <= 0.0f)))*((sd) + 0.5f) + (float((vBezier.x > 0.0f)))*((0.5f) - (sd)))), 0.0f, 1.0f));"+
